...
public class ConfigBean {
    ...
    public void populateDB() {
        ...
        if (policiesClientInd.size()>0) {
            occurrenceBean.create(clientInd.getUser_id(), 1, policiesClientInd.get(0).getId(), "Danos no carro após acidente","Submitted");
            occurrenceBean.create(clientInd.getUser_id(), 1, policiesClientInd.get(0).getId(), "Bicicleta roubada","Submitted");
        } else {
        ...
        if (policiesClientEmp.size()>0) {
            occurrenceBean.create(clientEmp.getUser_id(), 2, policiesClientEmp.get(0).getId(), "Acidente de trabalho, viatura danificada","Submitted");
            occurrenceBean.create(clientEmp.getUser_id(), 1, policiesClientEmp.get(0).getId(), "Incendio no armazem","Submitted");
        }else{
